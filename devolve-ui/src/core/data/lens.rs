...
/// data structure. The `Lens` trait is required for all datatypes
/// which you want to create lenses for.
///
/// For example, if you have a data structure, and a lens of its property,
/// you can set the property and it will reflect in the original data structure.
/// The real benefit of lenses is that they can be very deep (e.g. a property of a
/// property of an array index of another property), and you can pass them to children
/// so that the child can see / mutate only a part of your larger data structure.
///
/// Lenses in devolve-ui are special: you can also add observers which will be triggered
/// when the value itself or a property gets mutated. This is not technically a lens,
/// but the mutate / compositionality is used very often in devolve-ui.
///
/// The most common way to create `Lens` implementations is to
/// use [`#[derive(Lens)]`](devolve_ui_derive::::Lens) to access a struct's
...
pub trait Lens<Whole: ?Sized, Part: ?Sized> {
    ...
    fn with<V, F: FnOnce(&Part) -> V>(&self, data: &Whole, f: F) -> V;
    ...
    fn with_mut<V, F: FnOnce(&mut Part) -> V>(&self, data: &mut Whole, f: F) -> V;
}
...
pub trait LensExt<A: ?Sized, B: ?Sized>: Lens<A, B> {
    ...
    ///
    /// ```
    /// use devolve_ui::core::data::{widget::TextBox, Data, Lens, LensExt, Widget, WidgetExt};
    ///
    ...
    ///
    /// ```
    /// # use devolve_ui::core::data::*; use std::sync::Arc;
    ...
    /// ```
    /// # use devolve_ui::core::data::*;
    /// use devolve_ui::core::data::LensExt;
    ///
    /// #[derive(Lens)]
    /// struct MyThing {
    ///     first: bool
...
///
/// ```
/// let lens = devolve_ui::core::data::lens::Field::new(|x: &Vec<u32>| &x[42], |x| &mut x[42]);
/// ```
...
/// struct Foo { x: Bar }
/// struct Bar { y: [i32; 10] }
/// let lens = devolve_ui::core::data::lens!(Foo, x);
/// let lens = devolve_ui::core::data::lens!((u32, bool), 1);
/// let lens = devolve_ui::core::data::lens!([u8], [4]);
/// let lens = devolve_ui::core::data::lens!(Foo, x.y[5]);
/// ```
#[macro_export]
macro_rules! lens {
    ($ty:ty, [$index:expr]) => {
