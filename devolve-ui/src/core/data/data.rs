...
/// can be expensive. To use collection types, there are two easy options:
/// either wrap the collection in an `Arc`, or build devolve-ui with the `im` feature,
/// which adds `Data` implementations to the collections from the [`im` crate],
/// You can also build with `kurbo`, `piet`, and `shell` features (since code is from Druid)
///
/// ### Example:
///
/// ```
...
#[cfg(feature = "piet")]
impl_data_simple!(piet::InterpolationMode);
#[cfg(feature = "chrono")]
