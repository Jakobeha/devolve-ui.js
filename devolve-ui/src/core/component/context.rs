...
impl VContext {
    fn with_global<R>(f: impl FnOnce(Ref<VContext>) -> R) -> R {
        CONTEXT.with(|mut context: &RefCell<VContext>| f(context.borrow()))
    }
    ...
    fn with_global_subref_mut<R, UserR>(f: impl FnOnce(Ref<VContext>) -> &RefCell<R>, user_f: impl FnOnce(RefMut<R>) -> UserR) -> UserR {
        Self::with_global(|context| user_f(f(context).borrow_mut()))
    }

    fn with_global_opt_subref_mut<R, UserR>(f: impl FnOnce(Ref<VContext>) -> Option<&RefCell<R>>, user_f: impl FnOnce(Option<RefMut<R>>) -> UserR) -> UserR {
        Self::with_global(|context| user_f(f(context).map(|subref| subref.borrow_mut())))
    }

    unsafe fn with_global_subref_unsafe<R, UserR>(f: impl FnOnce(Ref<VContext>) -> &RefCell<R>, user_f: impl FnOnce(&mut R) -> UserR) -> UserR {
        Self::with_global(|context| user_f(f(context).as_ptr().as_mut().unwrap()))
    }
    ...
    pub fn with_push_renderer<R>(renderer: Weak<Renderer>, f: impl FnOnce() -> R) -> R {
        ...
        Self::with_global(|this| this.renderers.push(renderer));
        let result = f();
        Self::with_global(|this| this.renderers.pop());
        result
    }

    pub fn with_push_component<R>(component: Box<VComponent>, f: impl FnOnce() -> R) -> (R, Box<VComponent>) {
        // We need to not borrow during f or we'll get a RefCell runtime error
        Self::with_global_mut(|this| this.components.push(RefCell::new(component)));
        let result = f();
        let component = Self::with_global_mut(|this| this.components.pop().expect("component stack misaligned")).into_inner();
        (result, component)
    }

    pub fn with_clear_component_stack<R>(f: impl FnOnce() -> R) -> R {
        // We need to not borrow during f or we'll get a RefCell runtime error
        let mut old_components = Self::with_global_mut(|this| {
        ...
        Self::with_global_mut(|this| this.components.append(&mut old_components));
        result
    }
}
