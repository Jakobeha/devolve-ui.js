...
///
/// devolve_ui::core::misc::float_ord::sort(&mut v);
/// assert_eq!(v, [-5.0, -3.0, 1.0, 2.0, 4.0]);
/// ```
...
mod tests {
    ...
    fn test_ord() {
        ...
        assert_eq!(FloatOrd(1.0f64), FloatOrd(1.0f64));
        assert_eq!(FloatOrd(1.0f32), FloatOrd(1.0f32));
        assert!(FloatOrd(0.0f64) > FloatOrd(-0.0f64));
        assert!(FloatOrd(0.0f32) > FloatOrd(-0.0f32));
        assert_eq!(FloatOrd(f64::NAN), FloatOrd(f64::NAN));
        assert_eq!(FloatOrd(f32::NAN), FloatOrd(f32::NAN));
        assert!(FloatOrd(-f64::NAN) < FloatOrd(f64::NAN));
        assert!(FloatOrd(-f32::NAN) < FloatOrd(f32::NAN));
        assert!(FloatOrd(-f64::INFINITY) < FloatOrd(f64::INFINITY));
        assert!(FloatOrd(-f32::INFINITY) < FloatOrd(f32::INFINITY));
        assert!(FloatOrd(f64::INFINITY) < FloatOrd(f64::NAN));
        assert!(FloatOrd(f32::INFINITY) < FloatOrd(f32::NAN));
        assert!(FloatOrd(-f64::NAN) < FloatOrd(f64::INFINITY));
        assert!(FloatOrd(-f32::NAN) < FloatOrd(f32::INFINITY));
    }
    ...
    fn test_hash() {
        ...
        assert_ne!(hash(FloatOrd(f64::NAN)), hash(FloatOrd(-f64::NAN)));
        assert_ne!(hash(FloatOrd(f32::NAN)), hash(FloatOrd(-f32::NAN)));
        assert_eq!(hash(FloatOrd(f64::NAN)), hash(FloatOrd(f64::NAN)));
        assert_eq!(hash(FloatOrd(-f32::NAN)), hash(FloatOrd(-f32::NAN)));
    }
