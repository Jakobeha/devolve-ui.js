...
impl<'a, T: StableDeref> Iterator for Iter<'a, T> {
    ...
    fn next(&mut self) -> Option<&'a T::Target> {
        if let Some(ret) = self.0.get(self.idx) {
            self.idx += 1;
            Some(ret)
        } else {
            None
...
#[derive(Clone, Copy)]
pub enum SliceOrFrozenVec<'a, T> {
    Slice(&'a [T]),
    FrozenVec(&'a FrozenVec<T>)
}

pub enum SliceOrFrozenVecIter<'a, T> {
    Slice(std::slice::Iter<'a, T>),
    FrozenVec(Iter<'a, T>)
}

impl<'a, T: StableDeref> SliceOrFrozenVec<'a, T> {
    pub fn iter(&self) -> Iter<'a, T> {
        match self {
            SliceOrFrozenVec::Slice(s) => SliceOrFrozenVecIter::Slice(s.iter()),
            SliceOrFrozenVec::FrozenVec(s) => SliceOrFrozenVecIter::FrozenVec(s.iter())
        }
    }

    pub fn get(&self, idx: usize) -> Option<&'a T::Target> {
        match self {
            SliceOrFrozenVec::Slice(s) => s.get(idx),
            SliceOrFrozenVec::FrozenVec(s) => s.get(idx)
        }
    }
}

impl<'a, T: StableDeref>
