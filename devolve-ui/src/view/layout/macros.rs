pub macro _mt {
    ...
    ($mt:ident $sign:tt $lit:literal u $($rest:tt)*) => {{
        $mt = ($mt $sign $crate::view::layout::measurement::Measurement1 {
            value: $crate::view::layout::measurement::MeasurementValue::new(
                $lit as f32,
                $crate::view::layout::measurement::MeasurementDebugSymbol::Literal
            ),
            unit: $crate::view::layout::measurement::MeasurementUnit::Units,
    ...
    ($mt:ident $sign:tt $lit:literal px $($rest:tt)*) => {{
        $mt = ($mt $sign $crate::view::layout::measurement::Measurement1 {
            ...
            unit: $crate::view::layout::measurement::MeasurementUnit::Pixels,
        }).expect("measurement macro is too large");
        _mt!($mt $($rest)*)
    }},
    ($mt:ident $sign:tt $lit:literal % $($rest:tt)*) => {{
        $mt = ($mt $sign $crate::view::layout::measurement::Measurement1 {
            ...
            unit: $crate::view::layout::measurement::MeasurementUnit::PercentOfParent,
        }).expect("measurement macro is too large");
        _mt!($mt $($rest)*)
    }},
    ($mt:ident $sign:tt $lit:literal * prev $($rest:tt)*) => {{
        $mt = ($mt $sign $crate::view::layout::measurement::Measurement1 {
            ...
            unit: $crate::view::layout::measurement::MeasurementUnit::OfPrev,
        }).expect("measurement macro is too large");
        _mt!($mt $($rest)*)
    }},
    ($mt:ident $sign:tt $lit:literal * load($id:ident) $($rest:tt)*) => {{
        $mt = ($mt $sign $crate::view::layout::measurement::Measurement1 {
            ...
            unit: $crate::view::layout::measurement::MeasurementUnit::OfLoad($crate::misc::ident::Ident::try_from(stringify!($id)).expect("load ident is too large")),
        }).expect("measurement macro is too large");
        _mt!($mt $($rest)*)
    }},
    ...
    ($mt:ident $sign:tt {$exp:expr} u $($rest:tt)*) => {{
        $mt = ($mt $sign $crate::view::layout::measurement::Measurement1 {
            value: $crate::view::layout::measurement::MeasurementValue::new(
                $exp as f32,
                $crate::view::layout::measurement::MeasurementDebugSymbol::Expr(stringify!($exp))
            ),
            unit: $crate::view::layout::measurement::MeasurementUnit::Units,
        }).expect("measurement macro is too large");
        _mt!($mt $($rest)*)
...
pub macro mt {
    ($store:ident = $($rest:tt)*) => {
        {
            let mut mt = $crate::view::layout::measurement::Measurement::ZERO;
            mt.store = Some($crate::misc::ident::Ident::try_from(stringify!($store)).expect("store ident is too large"));
            _mt!(mt + $($rest)*)
        }
    },
    ($($rest:tt)*) => {
...
pub macro smt {
    (auto) => { $crate::view::layout::measurement::SizeMeasurement::AUTO },
    ($($rest:tt)*) => { $crate::view::layout::measurement::SizeMeasurement::from(mt!($($rest)*)) }
}
...
mod test {
    ...
    use crate::view::layout::macros::{_mt, mt, smt};
