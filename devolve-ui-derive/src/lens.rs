...
use crate::core::data::derive::attr::{FieldKind, Fields, LensAttrs};
...
fn derive_struct(input: &syn::DeriveInput) -> Result<proc_macro2::TokenStream, syn::Error> {
    ...
    let impls = fields.iter().filter(|f| !f.attrs.ignore).map(|f| {
        ...
        quote! {
            ...
            impl #impl_generics devolve_ui::core::data::Lens<#ty#ty_generics, #field_ty> for #twizzled_name::#field_name#lens_ty_generics #where_clause {
